...
data = pd.read_csv('dados/seccionador.csv', sep=",", encoding='ISO-8859-1', low_memory=False)

#Eliminando as colunas que não serão trabalhadas
data = util.clear_data(data)
...
#Aplicando a transformação one hot encoding que transforma colunas categóricas em
#colunas binárias (algoritmos de ML não entendem texto. Ex. transforma cor dos olhos em número)
...
#hue-campo usado para dar o tom (cor) dos pontos, vamos usar o campo de predição
ax = sns.relplot(x="tensao_nominal",y="localizacao",hue="tensao_nominal",data=data)
